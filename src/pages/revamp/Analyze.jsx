...
import { chatFollowUp } from "../../services/chatApi.js";
...
export default function Analyze({ incident, result, onProceed, onBack }) {
  ...
  const [pendingFile,   setPendingFile]   = useState(null); // { name, type }
  ...
  const handleFollowUp = async () => {
    ...
    const userMsg = { role: "user", text: q, file: pendingFile };
    setThread(t => [...t, userMsg]);
    setFollowInput("");
    setPendingFile(null);
    setFollowLoading(true);
    try {
      const res = await chatFollowUp(result?.session_id, q || `[attached: ${pendingFile?.name}]`);
      setThread(t => [...t, { role: "ai", text: res.answer }]);
    } catch {
      setThread(t => [...t, { role: "ai", text: "Unable to answer right now. Check the repair steps for guidance." }]);
    } finally {
  ...
  const handleFollowFile = e => {
    const f = e.target.files[0];
    if (f) setPendingFile({ name: f.name, type: f.type.startsWith("video") ? "video" : "image" });
    e.target.value = "";
  };
  ...
  return (
    <div className="r-page">
      <div className="r-page-center">
        ...
        <div className="r-split">
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div className="r-card r-card-accent">
              ...
              <p style={{ fontSize:13, color:"var(--text-muted)", marginBottom:16 }}>{fp.failure_area}</p>
              ...
              <div className="r-signal-list">
                {(fp.signals || []).map(s => (
                  <div className="r-signal-item" key={s}>
                    <div className="r-signal-dot" /><span>{s}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="r-signal-list">
                  {(visual.observed_indicators || []).map(i => (
                    <div className="r-signal-item" key={i}>
                      <div className="r-signal-dot visual" /><span>{i}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {mttr > 0 && (
              <div className="r-mttr-hero">
                <div>
                  ...
                  <p className="r-mttr-sub">{steps.length} steps · {supervised} supervised</p>
                </div>
              </div>
            )}
                ...
                <div className="r-signal-list">
                  {result.confidence_drivers.map(d => (
                    <div className="r-signal-item" key={d}>
                      <div className="r-signal-dot info" /><span>{d}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* Follow-up chat */}
        <div className="r-followup-section">
          <p className="r-card-title" style={{ marginBottom:14 }}>💬 Ask Vulcan a follow-up</p>

          {thread.length > 0 && (
            <div className="r-followup-thread">
              {thread.map((m, i) => (
                <div key={i} className={m.role === "user" ? "r-followup-q" : "r-followup-a"}>
                  {m.role === "ai" && <span className="r-followup-label">Vulcan AI</span>}
                  {m.text && <span>{m.text}</span>}
                  {m.file && (
                    <span className="r-attach-chip" style={{ display:"inline-flex", marginTop: m.text ? 6 : 0, fontSize:11 }}>
                      {m.file.type === "video" ? "🎥" : "📷"} {m.file.name}
                    </span>
                  )}
                </div>
              ))}
          ...
          {/* Staged file chip */}
          {pendingFile && (
            <div className="r-pending-bar" style={{ marginBottom:8 }}>
              <span style={{ fontSize:11, color:"var(--text-dim)", marginRight:6 }}>Attached:</span>
              <span className="r-pending-chip">
                {pendingFile.type === "video" ? "🎥" : "📷"} {pendingFile.name}
                <button className="r-pending-remove" onClick={() => setPendingFile(null)}>×</button>
              </span>
            </div>
          )}

          <div className="r-followup-bar">
            <label className="r-attach-btn" title="Attach photo or video" style={{ flexShrink:0 }}>
              <input ref={fileRef} type="file" accept="image/*,video/*" style={{ display:"none" }} onChange={handleFollowFile} />
              📎
            </label>
            <input
              className="r-followup-input"
              placeholder={`e.g. "What if I don't have a torque wrench?" or "Is this safe to defer overnight?"`}
              value={followInput}
            ...
            <button
              ...
              disabled={(!followInput.trim() && !pendingFile) || followLoading}
        ...
        <div className="r-actions" style={{ marginTop:24 }}>
